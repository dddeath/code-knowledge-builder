...
迁移保存生成器所有权清单之外的 Obsidian 文件、analysis/change/pitfall/experiment/session/user 页面、workspace notes、pending notes、session sidecar 和自动化数据库。SQLite 使用 backup API 复制后再执行当前 schema 初始化。固定源码 overlay 不从旧仓库继承，由目标仓库重新生成。
