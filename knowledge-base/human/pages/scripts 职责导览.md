# scripts 职责导览

标签：#类型/职责

> 这里汇集 scripts 范围内彼此协作的类、函数和辅助实现。

## 相关代码

- 继续浏览 [[CkbError 与 DependencyError 的协作实现]]。
- 继续浏览 [[SourceLinkRenderer.uri 与 SourceLinkRenderer 的协作实现]]。
- 继续浏览 [[_Transport.close 与 _StartGate 的协作实现]]。
- 继续浏览 [[__init__ 实现概览]]。
- 继续浏览 [[audit_agent_protocol 与 _default_python 的协作实现]]。
- 继续浏览 [[audit_feedback 与 _contains_chinese 的协作实现]]。
- 继续浏览 [[audit_gap_register 与 _root 的协作实现]]。
- 继续浏览 [[audit_migration 与 _entity_key 的协作实现]]。
- 继续浏览 [[audit_obsidian 与 prepare_vault 的协作实现]]。
- 继续浏览 [[audit_operation_journal 与 _root 的协作实现]]。
- 继续浏览 [[audit_output_contract 与 _default_ckb 的协作实现]]。
- 继续浏览 [[audit_work_record_index 与 _contains_chinese 的协作实现]]。
- 继续浏览 [[bind_conversation 与 default_management_registry_path 的协作实现]]。
- 继续浏览 [[check_fact_freshness 与 _root 的协作实现]]。
- 继续浏览 [[create_batch_plan 与 ProtocolRelease 的协作实现]]。
- 继续浏览 [[create_knowledge_batch_plan 与 KnowledgeRelease 的协作实现]]。
- 继续浏览 [[deployment_plan 与 skill_root 的协作实现]]。
- 继续浏览 [[doctor_report 与 _version_matches 的协作实现]]。
- 继续浏览 [[extract_pdf 与 PdfExtractionError 的协作实现]]。
- 继续浏览 [[finalize 与 _replace_output_prefix 的协作实现]]。
- 继续浏览 [[get_human_page_template 与 SectionContract 的协作实现]]。
- 继续浏览 [[ingest_event 与 default_registry_path 的协作实现]]。
- 继续浏览 [[ingest_reference 与 _root 的协作实现]]。
- 继续浏览 [[keyword_provider_config 与 parser 的协作实现]]。
- 继续浏览 [[load_page_config 与 _merge_known 的协作实现]]。
- 继续浏览 [[main 与 sha256 的协作实现]]。
- 继续浏览 [[main 相关实现]]。
- 继续浏览 [[maintenance_check 与 capability_matrix 的协作实现]]。
- 继续浏览 [[module_name 与 estimated_tokens 的协作实现]]。
- 继续浏览 [[package_showcase 与 _parse_sample 的协作实现]]。
- 继续浏览 [[parse_file 与 _language 的协作实现]]。
- 继续浏览 [[preflight 与 git 的协作实现]]。
- 继续浏览 [[propose_template 与 _canonical_bytes 的协作实现]]。
- 继续浏览 [[query_graph 与 _networkx_modules 的协作实现]]。
- 继续浏览 [[record_note 与 page_tag 的协作实现]]。
- 继续浏览 [[register_obsidian_plugin 与 default_obsidian_plugin_registry 的协作实现]]。
- 继续浏览 [[render_integration 与 harness_retrieval_contract 的协作实现]]。
- 继续浏览 [[render_page_author 与 _error 的协作实现]]。
- 继续浏览 [[replace_note 与 RecordReplaceLockError 的协作实现]]。
- 继续浏览 [[retrieve 与 _tokens 的协作实现]]。
- 继续浏览 [[retrieve_machine 与 estimated_tokens 的协作实现]]。
- 继续浏览 [[run_keyword_benchmark 与 _text_list 的协作实现]]。
- 继续浏览 [[run_keyword_provider 与 KeywordProviderConfig 的协作实现]]。
- 继续浏览 [[search_terms 与 _split_camel 的协作实现]]。
- 继续浏览 [[serve_stdio 与 _write_line 的协作实现]]。
- 继续浏览 [[source_files 与 sha256 的协作实现]]。
- 继续浏览 [[start_scope_extension 与 _error 的协作实现]]。
- 继续浏览 [[start_session 与 _session_directory 的协作实现]]。
- 继续浏览 [[sync_human_layer 与 _source_manifest 的协作实现]]。
- 继续浏览 [[validate_human_maintenance_invocation 与 ParameterSpec 的协作实现]]。
- 继续浏览 [[web_input_adapter_contract 与 ReferenceInputRequest 的协作实现]]。

## 谁会来到这里

- 可从 [[source 代码导览]] 进入本页。
