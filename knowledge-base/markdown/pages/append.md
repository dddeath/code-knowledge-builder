...
> `ManagementSchemaPersi...` 是 `tests/test_management_agent.py` 第 111-113 行定义的函数，本页绑定该固定源码范围。 该函数作为可执行验证入口，检查标识符 `append` 所指的行为与失败边界。

## 什么时候需要修改

当被测行为、输入夹具、断言或失败条件变化时，应同步更新 `append` 的说明。

## 在代码中的位置

[打开源码：tests/test_management_agent.py 第 111 行](vscode://file/E:/knowledge_builder/self-workspace/source/tests/test_management_agent.py:111:1)  `tests/test_management_agent.py:111-113`
