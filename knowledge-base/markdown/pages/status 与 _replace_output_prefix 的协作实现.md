...
> 该文件实现从范围解析、分段构建、审阅、合并到双知识层投影和最终审计的主流水线。 它统一维护固定快照、页面配置、迁移目录提升后的路径重定位、机器与人类投影及完成状态机。

## 什么时候需要修改

构建阶段、审计门、投影格式、完成状态或输出目录迁移契约变化时，需要修改该文件。
...
<details><summary>查看本页收纳的 67 个辅助实现</summary>

| 代码单元 | 一句话作用 |
|---|---|
| `_replace_output_prefix` | 递归改写 JSON 中属于旧输出目录的普通、正斜杠和 URI 编码路径前缀。 |
| `_relocate_completed_output` | 在 staging 目录被同卷提升后验证本地固定快照，并重定位输出自有 JSON 路径且保留不可变基线原字节。 |
| `_load_state` | 读取状态、兼容旧 chunks 字段、处理已完成输出重定位，并验证固定快照与页面配置。 |
