...
### Skill-only retrieval when project instructions were not injected

If the Harness has loaded this exact `code-knowledge-builder` Skill body but did not inject the project's `AGENTS.md`, `CLAUDE.md`, `GEMINI.md`, Copilot instructions, or Cursor rule, use this Skill as the retrieval entry. Do not treat a missing project-instruction adapter as evidence that CKB is unavailable.

1. Resolve one project binding without scanning directories. Use the first unambiguous source in this order: explicit `REPO`/`OUTPUT`/`PYTHON`/`CKB` values in the current task or handoff; an already supplied manager identity through `manager status/context`; the exact current repository or workspace match returned by `automation registry`. Reopen `OUTPUT/state.json`, require `OUTPUT/machine/knowledge.sqlite`, and confirm that the bound repository is the task repository before querying.
2. Project registration and session activation are needed for event sync and resident stdio, not for direct retrieval. When the project is registered and the Harness exposes a stable session ID, run `automation activate --harness HARNESS --session-id SESSION_ID --cwd TASK_ROOT --registry REGISTRY`. If the adapter, session ID, or resident process is absent, continue through the same per-command CLI and report the returned `cli-fallback` or direct-CLI mode rather than claiming resident stdio.
3. Execute `brief --profile fast`, reopen both returned `pack` and `record`, list open feedback when `open_feedback > 0`, and use `entity`、`neighbors`、`source` or `changes` only for the returned selector/path/range. A `needs-source-read` result triggers bounded source inspection; it is not a successful fact match.
4. Query commands persist Agent packs, freshness state, and bounded operation-journal records. When the bound knowledge base is declared read-only, run the closure only on an explicitly designated writable isolated copy and leave the stable output unchanged.
5. `record` and `maintain` are task-triggered. Create a durable record only when the user or task contract asks to retain/synchronize the conclusion. Run `maintain` after a requested knowledge-base mutation or when maintenance status is itself an acceptance result. A missing adapter for another Harness may fail the maintenance audit, but it does not block `brief`, feedback, gaps, or narrow source reads for the active task.

If the exact Skill body was not loaded, do not emit `skill.applied`, do not start resident stdio, and do not describe the CKB workflow as applied. A plain project-name mention is not activation. The host-source matrix and exact invocation forms are in [references/agent-policy.md](references/agent-policy.md); the runtime probe is `tests/harness_retrieval_contract_probe.py`.
...
- For a completed knowledge base that needs another center on the same fixed Git snapshot, use `scope extend start/status/audit/cutover/rollback`; the command forms an append-only union in isolated staging, reuses exact blobs/reviews, and preserves a byte-verifiable rollback. When `retrieve` or `brief` has insufficient evidence and returns a `scope_extension_offer`, show its Chinese confirmation prompt first; run the enclosed existing `scope extend start` command only after confirmation. Read [references/scope-extension.md](references/scope-extension.md) before extending or promoting a scope.
- If `OUTPUT/machine/knowledge.sqlite` exists and the user asks an architecture, explanation, or change-location question, use `brief --profile fast` first and open only its budgeted Agent pack. `brief` keeps a complete retrieval record when a pack exists, but omits candidate entities, terms, scores, and relation details from the first command response. Use `retrieve --profile precise` for harder queries, `entity` for exact symbols, `neighbors` for bounded graph expansion, `source` for an exact source range, and `changes` for durable Agent records. Use narrow source reading only when the compact result requests a source fallback. A scope offer is only a human decision point; when it is deferred, continue the returned narrow source read and do not repeat the same `offer_id` in one caller context.
...
When the user or task contract requires the explanation or analysis to be retained in the knowledge base, open the returned pack, write the analysis body, and record it before replying. Ordinary read-only answers do not create a record:
...
- Several complete older OUTPUTs use the explicit-manifest `migrate batch plan/apply/resume/status/audit/cutover/rollback` family. Planning is read-only and accepts only the audited historical CKB/Schema/Agent Protocol matrix. Each project builds in an independent staging directory, preserves every mutable knowledge layer, upgrades internal Agent Protocol there, and stops at `review-pending` for exact delta or cold-build reviews. Only a project that passes current retrieval, three completion markers, both SQLite databases, human/Markdown parity, readability, reference, gap, operation journal, Agent Policy and `maintain` may cut over. Cutover and rollback use per-OUTPUT owner-token locks, independent byte manifests, external workspace-protocol backups, deterministic parent chains and subset recovery. Read [references/knowledge-base-batch-migration.md](references/knowledge-base-batch-migration.md) before migrating more than one complete OUTPUT.
