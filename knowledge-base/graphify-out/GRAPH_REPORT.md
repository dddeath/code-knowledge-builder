...
- **status**：`status` 返回当前构建、审阅、完成门和下一步动作。
- **main**：`main` 解析命令并把请求分派到对应的确定性实现。
- **CodeKnowledgeBuilderTests**：`CodeKnowledgeBuilderTests` 汇集 code-knowledge-builder 的端到端验收场景。
...
- **retrieve_machine**：`retrieve_machine` 根据问题生成来源绑定且受预算约束的 Agent 阅读包。
...
- **source_files**：`source_files` 按发行类型返回稳定排序的待归档文件集合。
- **main**：`main` 是源码中负责接收命令参数并把请求路由到对应实现的命名代码单元。
